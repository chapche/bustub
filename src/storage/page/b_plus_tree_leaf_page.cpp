...
void B_PLUS_TREE_LEAF_PAGE_TYPE::InsertAt(int index, const MappingType &pair) {
  ...
  int size = GetSize();
  int max_leaf_page_size = static_cast<int>(LEAF_PAGE_SIZE);
  if (size >= max_leaf_page_size) {
    LOG_WARN("szie(%d) exceeds max_leaf_page_size(%d)!", size, max_leaf_page_size);
    // size = max_leaf_page_size - 1;
  }
